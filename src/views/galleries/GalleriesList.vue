          ...
          >
            <td class="px-4 py-3 align-middle">
              <button class="flex items-center gap-3 text-left" @click="goEdit(s.id)">
                ...
                <span class="text-[14.5px] font-600 text-graphite-900 group-hover:text-brand-600">{{ s.name.cs }}</span>
              </button>
            </td>
            <td class="px-2 py-3 align-middle">
              <span class="block max-w-[420px] truncate text-[13px] text-steel-500">{{ plain(s.description.cs) || '—' }}</span>
            </td>
            <td class="px-2 py-3 align-middle">
              <span class="inline-flex items-center gap-1.5 rounded-full bg-steel-100 px-2.5 py-1 text-[12px] font-600 text-graphite-700">
                <Icon name="gallery" :size="13" class="text-steel-400" /> {{ galleriesInSection(s.id).length }}
              </span>
            </td>
            <td class="px-2 py-3 align-middle">
